...
function global:Update {
    ...
    try {
        ...
        if (($latestVersion -ne $ver) -and ("" -ne $latestVersion)) {
            Debuging -param_debug $debug -debugmessage ("New version available. LOADING... - ($latestVersion)") -typemessage info
            # load file
            $zipUrl = $repoUrl + "/archive/$latestVersion.zip"
            $zipPath = ".\update.zip"
            Invoke-WebRequest -Uri $zipUrl -OutFile $zipPath
            Debuging -param_debug $debug -debugmessage ("Loading file!: -  ($zipUrl)") -typemessage info 
        ...
        elseif ($latestVersion -eq $ver) {
            Debuging -param_debug $debug -debugmessage ("Script version is current - ($ver)") -typemessage info 
